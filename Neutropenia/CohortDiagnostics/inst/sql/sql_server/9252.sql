...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (440689)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (440689)
  and c.invalid_reason is null
...
WHERE (C.value_as_number >= 0.0100 and C.value_as_number <= 0.1990)
...
WHERE (C.value_as_number >= 10.0000 and C.value_as_number <= 199.0000)
